...
def user_actif():
    """La fonction user_actif() permet de savoir quels sont les utilisateurs logger sur le serveur.

    """
    global user
    global user_connecté
...
def deplacer_user():
    ...
    if user_connecté == True:
        print(user + " est toujours logger sur le système, vous devez fermer sa session!")
        sys.exit(0)
